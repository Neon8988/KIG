...
from src.mol_metrics import Tokenizer
from src.discriminator import DiscriminatorModel
from data.data_preprocess import DataPreprocessor
from src.filler import FillerModel, FillerSampler
from src.rollout import DeepCopiedFiller, Rollout
from data.data_iter import FillerDataLoader, DisDataLoader
...
def train(args):
    ...
    SCAF_MIN_LEN = args.scaf_min_len
    ...
    with open(PATHS + '/hyperparameters.csv', 'a+') as hp:
        ...
        params['SCAF_MIN_LEN'] = SCAF_MIN_LEN
    ...
    # Data preprocessing
    if PREPROCESS:
        print('Double check data preprocessing...')
        return
    ...
    if not os.path.isfile(PREPROCESSED_FILE):
        ...
        smiles = smiles.sample(FILLER_GENERATED_SIZE,random_state=0)
        smiles.to_csv(POSITIVE_FILE,index=False)
    
    with open(PREPROCESSED_FILE, encoding='utf-8') as f:
        TOTAL_SIZE = sum(1 for _ in f)
    print('\nWill use training data saved in : {}.\n'.format(PREPROCESSED_FILE))
    ...
    if FILLER_PRETRAIN:
        ...
        print(f'\nFiller Pretrain Time:{int(current_time//3600)}h {int((current_time%3600)//60)}m {current_time%60:.2f}s')
    ...
    filler_sample_loader=FillerDataLoader(
        tokenizer,
        POSITIVE_FILE, 
        0,
        0,
    ...
    sample_loader = filler_sample_loader.sample_dataloader(POSITIVE_FILE)
    ...
    if DIS_PRETRAIN:
        ...
        print(f'Discriminator Pretrain Time: {(time.time() - start_time) / 3600.:.2f} hours')
    ...
    updated_filler_loader.setup(shuffle=False)       
    negative_loader = updated_filler_loader.train_dataloader(shuffle=False)

    # Adversarial training
    if ADVERSARIAL_TRAIN:
        print('\nAdversarial Training...')
        negative_cycle = cycle(negative_loader)
        for epoch in range(ADV_EPOCHS):
            ...
            batch_scaffold_input, batch_decoration_input = next(negative_cycle)
            ...
            # Save models
            torch.save(filler.state_dict(), PATHS + '/Epoch_' + str(epoch+1) + '_filler.pt')
            if DIS_LAMBDA:
                torch.save(dis.state_dict(), PATHS + '/Epoch_' + str(epoch+1) + '_dis.pt')
            # Generate Samples
            filler_sampler =  FillerSampler(
                filler, 
                tokenizer, 
            ...
            current_time = (time.time() - start_time)
            print(f'\nTotal Computational Time: {int(current_time//3600)}h {int((current_time%3600)//60)}m {current_time%60:.2f}s')
            evaluation(
                POSITIVE_FILE,
                NEGATIVE_FILE, 
                FILLER_GENERATED_SIZE, 
