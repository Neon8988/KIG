...
parser.add_argument('--scaf_min_len', type=int, default=5, help='minimum length of the scaffolds')
...
parser.add_argument('--dataset_name', type=str, default='kinase_organic', help='use kinase_organic, QM9_10k or ZINC_10k dataset')
...
parser.add_argument('--filler_dim_feedforward', type=int, default=128, help='define the dimension of the feedforward layer of the filler model')
...
parser.add_argument('--dis_dim_feedforward', type=int, default=256, help='define the dimension of the feedforward layer of the discriminator model')
...
parser.add_argument('--dis_lambda', type=float, default=0.2, help='define the tradeoff between RL and GAN')
parser.add_argument('--adv_lr', type=float, default=2e-5, help='define the learning rate for the GAN or WGAN')
parser.add_argument('--whole_smiles', action='store_true', help='whether using the complete SMILES strings')
parser.add_argument('--properties', type=str, default='kinase_properties', help='define the chemical property for molecular generation',choices=['kinase_properties', 'kinase_fsp3', 'kinase_clogp', 'kinase_tpsa', 'kinase_multi_objective'])
parser.add_argument('--fsp3_weight', type=float, default=1.0, help='weight for FSP3 in multi-objective')
parser.add_argument('--clogp_weight', type=float, default=1.0, help='weight for clogP in multi-objective')
parser.add_argument('--tpsa_weight', type=float, default=1.0, help='weight for TPSA in multi-objective')
...
PREPROCESSED_FILE = 'datasets/preprocessed_' + args.dataset_name + '_LEN_' + str(args.dec_min_len) + '_SCAF_' + str(args.scaf_min_len) + '.csv'
POSITIVE_FILE = 'datasets/' + args.dataset_name + '_LEN_' + str(args.dec_min_len) + '_SCAF_' + str(args.scaf_min_len) + str(args.filler_generated_size) + '.csv'
NEGATIVE_FILE = 'results/generated_smiles_' + args.dataset_name + '_EPOCH_' + str(args.filler_epochs)  + str(args.filler_generated_size) + '.csv'
FILLER_PRETRAINED_MODEL = 'results/save_models/pretrained_filler_' + args.dataset_name + '_EPOCH_' + str(args.filler_epochs) + '.pt'
DIS_PRETRAINED_MODEL = 'results/save_models/pretrained_discriminator_' + args.dataset_name + '_EPOCH_' + str(args.dis_pre_epochs) + '.pt'
LEFT_PROPERTIES = ['kinase_properties', 'kinase_fsp3', 'kinase_clogp', 'kinase_tpsa', 'kinase_multi_objective']
...
ScaffGAN_FILLER_MODEL = PATHS + '/Epoch_' + str(args.save_name) + '_filler.pt' 
ScaffGAN_DIS_MODEL = PATHS + '/Epoch_' + str(args.save_name) + '_dis.pt'
...
parser.add_argument('--scaffgan_filler_model', type=str, default=ScaffGAN_FILLER_MODEL, help='the path for saving the trained filler model of the KIG')
parser.add_argument('--scaffgan_dis_model', type=str, default=ScaffGAN_DIS_MODEL, help='the path for saving the trained discriminator model of the KIG')
parser.add_argument('--top_12_gan', type=str, default=TOP_12_GAN, help='the path for saving the top 12 molecular structures of the KIG')
parser.add_argument('--top_12_wgan', type=str, default=TOP_12_WGAN, help='the path for saving the top 12 molecular structures of the KIG')
parser.add_argument('--draw_distributions', type=str, default=DRAW_DISTRIBUTIONS, help='the path for saving the distributions of the KIG')
...
parser.add_argument('--trained_filler', type=str, default='results/Epoch_50_filler.pt', help='the path to the trained filler model')
