    ...
    def get_reward(
        ...
        for i in range(rollout_num):
            ...
            for given_num in range(init, seq_len):
                ...
                # Use appropriate encoding: scaffold_encode for whole SMILES, decoration_encode for decorations only
                encode_fn = self.tokenizer.scaffold_encode if self.whole_smiles else self.tokenizer.decoration_encode
                generated_smiles_encoded = [torch.tensor(encode_fn(smi))[1:-1] for smi in generated_smiles]
            ...
            encode_fn = self.tokenizer.scaffold_encode if self.whole_smiles else self.tokenizer.decoration_encode
            last_encoded = [torch.tensor(encode_fn(s))[1:-1] for s in sample_smiles]
            ...
            if i == 0:
                rewards.insert(0, dis_pred)  # Insert at beginning for position 1
            else:
                rewards[0] += dis_pred  # Add to position 1

        rewards = np.transpose(np.array(rewards)) / (1.0 * rollout_num) # [batch_size, seq_len-1]
        # Debugging reward statistics
        print(f"Raw rewards before baseline: mean={np.mean(rewards):.4f}, std={np.std(rewards):.4f}")
        print(f"Raw rewards range: [{np.min(rewards):.4f}, {np.max(rewards):.4f}]")
