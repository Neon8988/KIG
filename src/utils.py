...
def evaluation(
    ...
    if len_valid:
        ...
        print('Metrics of target property for both decorations and the complete molecules:')
        decoration_vals = reward_fn(properties, novel_smiles['decorations'])
        mean_dec, std_dec, min_dec, max_dec = np.mean(decoration_vals), np.std(decoration_vals), np.min(decoration_vals), np.max(decoration_vals)
        print('[{}]: [Mean: {:.3f}   STD: {:.3f}   MIN: {:.3f}   MAX: {:.3f}]'.format('decorations', mean_dec, std_dec, min_dec, max_dec))
        # Compute the generated SMILES scores
        vals = reward_fn(properties, novel_smiles['smiles'])
        mean_s, std_s, min_s, max_s = np.mean(vals), np.std(vals), np.min(vals), np.max(vals)
        print('The whole molecule:[target_property:{}]: [Mean: {:.3f}   STD: {:.3f}   MIN: {:.3f}   MAX: {:.3f}]'.format(properties, mean_s, std_s, min_s, max_s))
        
        print('Metrics of remaining properties for the complete molecules:')
        for untarget_property in left_properties:
            if untarget_property == 'kinase_multi_objective':
                continue
            vals = reward_fn(untarget_property, novel_smiles['smiles'])
            mean_s, std_s, min_s, max_s = np.mean(vals), np.std(vals), np.min(vals), np.max(vals)
            print('[{}]: [Mean: {:.3f}   STD: {:.3f}   MIN: {:.3f}   MAX: {:.3f}]'.format(untarget_property, mean_s, std_s, min_s, max_s))
    else:
