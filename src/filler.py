...
class FillerModel(LightningModule):
    ...
    def __init__(
        ...
        self.setup_layers()
        # Don't pre-calculate steps_per_epoch - let PyTorch Lightning determine it automatically
    ...
    def configure_optimizers(self):    
        ...
        # Calculate total_steps using a more compatible approach
        # Use a conservative estimate for steps per epoch to avoid division by zero
        ...
        scheduler = torch.optim.lr_scheduler.OneCycleLR(
            ...
            pct_start=min(0.25, 4/self.epochs),  # Ensure pct_start is between 0 and 1
            anneal_strategy='cos', 
            cycle_momentum=True, 
            base_momentum=0.85, 
            max_momentum=0.95,
...
class FillerSampler():
    ...
    def decoration_sampler(self, scaffold_input, data=None):
        ...
        # Ensure the last token is the end token
        sample_tensor[:,-1]= self.tokenizer.char_to_int[self.tokenizer.mask_end]
